...
st.set_page_config(page_title="DARELL V5 PRO MAX", layout="centered", page_icon="📈")
...
with tab0:
    ...
    if st.button("👆 Tap to analyze", use_container_width=True, type="primary"):
        ...
        if rsi < 30:
            ...
            st.error("🔴 RSI Overbought - SELL SIGNAL")
        else:
            st.info("🟡 RSI Neutral - Wait")
...
with tab1:
    ...
    st.caption("For your pairs: BTC, GOLD, ETH, BNB, BCH, etc.")
    ...
    if st.button("Calculate Profit", use_container_width=True, type="primary"):
        ...
        if profit > 0:
            st.success(f"✅ PROFITABLE! Kita ka ng ${profit:,.2f}")
        else:
            st.error(f"❌ LOSS ng ${profit:,.2f}")

with tab2:
    st.markdown("### 😱 Fear & Greed Index")
    # Simple calculation based on RSI average
    try:
        rsi_avg = []
    ...
    if fg_value < 25:
        ...
        st.progress(fg_value, text="Extreme Fear")
    elif fg_value < 45:
        st.markdown("<p class='fear' style='text-align:center'>Fear</p>", unsafe_allow_html=True)
        st.progress(fg_value, text="Fear")
    elif fg_value < 55:
        st.markdown("<p style='text-align:center'>Neutral</p>", unsafe_allow_html=True)
        st.progress(fg_value, text="Neutral")
    elif fg_value < 75:
        st.markdown("<p class='greed' style='text-align:center'>Greed - Market going up</p>", unsafe_allow_html=True)
        st.progress(fg_value, text="Greed")
    else:
        st.markdown("<p class='greed' style='text-align:center'>Extreme Greed - Be Careful!</p>", unsafe_allow_html=True)
        st.progress(fg_value, text="Extreme Greed")

    st.info(f"Based on RSI of BTC, ETH, BNB: {fg_value}. 0=Fear, 100=Greed")
    ...
    if st.session_state['alerts']:
        ...
        for i, a in enumerate(st.session_state['alerts']):
            try:
                ...
                if triggered:
                    st.error(f"🚨 TRIGGERED! {a['pair']} now ${curr:,.2f} - {a['type']}")
                    st.balloons()
                else:
                    st.info(f"{a['pair']} | Target: ${a['target']:,.2f} | Now: ${curr:,.2f} | {a['type']}")
            except:
...
with tab4:
    ...
    if st.button("Get Live Indicators", use_container_width=True, type="primary"):
        ...
        c1,c2 = st.columns(2)
        c1.metric("RSI 14", f"{rsi:.2f}", "Oversold" if rsi<30 else "Overbought" if rsi>70 else "Neutral")
        c1.metric("MACD", f"{macd:.4f}")
        c1.metric("Signal", f"{sig:.4f}")
        c2.metric("SMA 20", f"${sma:,.2f}")
        c2.metric("Upper Boll", f"${up:,.2f}")
        c2.metric("Lower Boll", f"${low:,.2f}")
        
        st.markdown(f"**Support:** ${sup:,.2f} | **Resistance:** ${res:,.2f}")

st.caption("2026 DARELL V5 PRO MAX - Calculator + Indicators + FearGreed + Alerts")
